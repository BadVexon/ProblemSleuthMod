...
public class InsertCardAction implements RenderSubscriber, PostUpdateSubscriber {
    ...
    private AbstractCard cardToStore;

    public InsertCardAction(AbstractRelic r, float originX, float originY, AbstractCard card) {
        this.r = r;
        this.origin = new Vector2(originX, originY);
        this.cardToStore = card;
    ...
    public InsertCardAction(AbstractRelic r, Hitbox origin, AbstractCard card) {
        this(r, origin.cX, origin.cY, card);
    }

    public InsertCardAction(AbstractRelic r, AbstractCard card) {
        this(r, ((AbstractRelic) r).hb, card);
    }

    private void close() {
        this.isHidden = true;
        GameCursor.hidden = false;
        InsertCardAction vex = this;
    ...
    private void updateTargetMode() {
        ...
        if (InputHelper.justClickedLeft) {
            if (this.hoveredCreature != null && this.hoveredCreature != AbstractDungeon.player) {
                AbstractDungeon.actionManager.addToTop(new ApplyPowerAction(this.hoveredCreature, AbstractDungeon.player, new StasisPower(this.hoveredCreature, cardToStore), 1));
                close();
            }
        }
    }
